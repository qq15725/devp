...
import { certDir, rootCACrtPath, rootCAKeyPath } from '@netproxy/shared'
...
export function getRootCa() {
  return {
    cert: cert ??= fs.readFileSync(rootCACrtPath, { encoding: 'utf8' }),
    key: key ??= fs.readFileSync(rootCAKeyPath, { encoding: 'utf8' }),
  }
}
...
export function checkRootCA() {
  ...
  if (!(fs.existsSync(rootCACrtPath) && fs.existsSync(rootCAKeyPath))) {
...
export function generateRootCA(commonName: string) {
  return new Promise(resolve => {
    if (!fs.existsSync(rootCACrtPath) || !fs.existsSync(rootCAKeyPath)) {
      ...
      fs.writeFileSync(rootCACrtPath, certificate)
      fs.writeFileSync(rootCAKeyPath, privateKey)
      consola.success('rootCA generated')
      consola.success(`PLEASE TRUST the rootCA.crt in ${ certDir }`)
    }
    resolve({ keyPath: rootCAKeyPath, certPath: rootCACrtPath })
  })
}
